...
class OpticalFlowDetection(Vprocess):
    def __init__(self, shared_dict: DictProxy, lock: Lock, roi: Tuple[int, int, int, int]) -> None:
        ...
        self._back_sub = cv2.createBackgroundSubtractorMOG2(history=50, varThreshold=10, detectShadows=True)
    ...
    def load_params(self) -> None:
        ...
        params_file = os.path.join(current_dir, 'optical_flow_detection.yaml')
    ...
    def calculate_flow(self, frame, time_stamp: datetime):
        ...
        if 95<bg_percentage:
            cv2.putText(frame, f'Could not detect flow', (10, 30),
        ...
        flow = cv2.calcOpticalFlowFarneback(prvs_frame_roi, next_frame_roi, None, **self._flow_params)
        ...
        flow_y = flow[..., 1]
        y_speed_per_second = np.mean(np.abs(flow_y)[flow_y != 0])/(time_stamp-self._last_time_stamp).total_seconds()
        self._last_time_stamp = time_stamp
        ...
        frame[y:y+h, x:x+w] = cv2.addWeighted(frame[y:y+h, x:x+w], 0.5, bgr_roi, 0.5, 0)
        cv2.putText(frame, f'Y Speed: {self._current_speed:.2f} pixels/second', (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        ...
        while True:
            try:
                ...
                if self._current_frame_number != 0:
                    self._processed_frame = self.calculate_flow(self._current_frame, self._current_frame_time) if change else self._processed_frame
